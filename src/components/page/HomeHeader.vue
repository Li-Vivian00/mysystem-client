<template>
  <div class="homeHeader">
    <div class="logo">
      <img src="../../../static/img/logo.jpg" alt="The Shield" />
    </div>
    <div class="menuBox">
      <el-menu
        ...
        @select="handleSelect"
      >
        <el-menu-item index="homeCenter">{{
          $t("homePage.homeHeader.home")
        }}</el-menu-item>
        <el-menu-item index="aboutUs">{{
          $t("homePage.homeHeader.aboutUs")
        }}</el-menu-item>
        <el-menu-item index="dailyNews">{{
          $t("homePage.homeHeader.dailyNews")
        }}</el-menu-item>
        <el-submenu index="3" v-if="username == ''">
          <template slot="title">{{
            $t("homePage.homeHeader.login")
          }}</template>
          <el-menu-item index="adminLogin">{{
            $t("homePage.homeHeader.adminLogin")
          }}</el-menu-item>
          <el-menu-item index="/">{{
            $t("homePage.homeHeader.userLogin")
          }}</el-menu-item>
        </el-submenu>
        <el-submenu index="userHome" v-else>
          <template slot="title" style="border-bottom: 2px solid #fff;">{{
            $t("header.userCenter")
          }}</template>
      ...
      <div class="langBox">
        <span class="item" v-if="username != null">
          {{ username }}
        </span>
        <img src="../../../static/global.svg" alt="" class="global" />
        <el-dropdown @command="selectLang" class="language">
