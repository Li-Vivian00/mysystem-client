<template>
  <div class>
    ...
    <el-table :data="form"
      ...
      <el-table-column prop="floor_id"
                       :label='`${$t("buildingManage.floorId")}`'
                       sortable></el-table-column>
      <el-table-column prop="floor_name"
      ...
      <el-table-column prop="end_date"
                       ...
                       sortable></el-table-column>
      <el-table-column prop="floor_hold"
                       :label='`${$t("buildingManage.floorHold")}`'
                       sortable></el-table-column>
      <el-table-column prop="floor_area"
      ...
      <el-table-column fixed="right"
        ...
        <template slot-scope="scope">
          <div>
            <a href="#"
               v-for="item in optionsA"></a>
          </div>
          <el-select v-model="selectValue"
                     clearable
                     placeholder="请选择"
                     @change="selectBuilding">
            <el-option v-if="scope.row.floor_id == 'A'"
                       v-for="item in optionsA"
                       :key="item.value"
                       :label="item.label"
                       :value="item.value"></el-option>
            <el-option v-if="scope.row.floor_id == 'B'"
                       v-for="item in optionsB"
                       :key="item.value"
                       :label="item.label"
                       :value="item.value"></el-option>
            <el-option v-if="scope.row.floor_id == 'C'"
    ...
    <el-table :data="form.slice((currentPage-1) * pagesize, currentPage * pagesize)"
      ...
      <el-table-column prop="room_id"
                       ...
                       sortable></el-table-column>
      <el-table-column prop="room_type"
                       :label='`${$t("buildingManage.roomType")}`'
                       sortable></el-table-column>
      <el-table-column prop="room_area"
                       :label='`${$t("buildingManage.roomArea")}`'
                       sortable></el-table-column>
      <el-table-column prop="stay_date"
