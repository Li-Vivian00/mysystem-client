<template>
  <div class="billManage">
    <el-button type="primary" @click="clickAddBill" class="batchDelect">{{
      $t("billManage.addBill")
    }}</el-button>
    <div class="headerStyle">
      ...
      <el-button type="primary" @click="searchBill" class="searchUser" plain>{{
        $t("manage.search")
      }}</el-button>
    </div>
    <el-table
    ...
    >
      ...
      ></el-table-column>
      <el-table-column
        :label="`${$t('repairManage.status')}/${$t('manage.operate')}`"
        width="150px"
      >
          ...
          <span v-if="scope.row.status == 0"
            ...
            <el-tooltip :content="`${$t('billManage.detail')}`" placement="top">
              <el-button
                type="text"
                size="small"
                @click="showBillDetail(scope.row)"
    ...
    >
      ...
      >
        ...
        <el-form-item prop="room_id" :label="`${$t('register.label.room_id')}`">
          <el-input
            v-model.trim="editForm.room_id"
            auto-complete="off"
            class="inputWidth"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item prop="type" :label="`${$t('repairManage.type')}`">
          <el-select
            class="inputWidth"
          ...
          >
            <el-option
              :label="`${$t('billManage.waterRent')}`"
              :value="`${$t('billManage.waterRent')}`"
            ></el-option>
            <el-option
              :label="`${$t('billManage.powerRent')}`"
              :value="`${$t('billManage.powerRent')}`"
            ></el-option>
            <el-option
              :label="`${$t('billManage.managementFee')}`"
              :value="`${$t('billManage.managementFee')}`"
            ></el-option>
            <el-option
              :label="`${$t('billManage.garbageCharge')}`"
              :value="`${$t('billManage.garbageCharge')}`"
            ></el-option> </el-select
          ><br />
          {{ $t("billManage.price")
          }}<el-input-number
          ...
          {{ $t("billManage.count")
            ...
            size="small"
          ></el-input-number>
          <p v-show="editForm.type != ''" style="margin: 0px; text-align: left">
            {{ editForm.type }}: {{ editForm.price }} * {{ editForm.count }} =
            {{ total }}
          </p>
        </el-form-item>
        ...
        <el-form-item :label="`${$t('billManage.remark')}`" prop="remark">
          <el-input
            type="textarea"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitBill('editForm')">{{
          $t("button.add")
        }}</el-button>
      </div>
    </el-dialog>
...
import {
  ...
  deleteBill,
  getBillById
} from "../../../service/admin/billManage/billManage.Service";
import {
  validateName,
  validateLoginId,
  validatePhone,
        ...
        room_id: [
          {
            required: true,
            validator: (rule, value, callback) =>
              validateRoomId(rule, value, callback, self),
            trigger: "blur"
          }
        ],
        phone: [
          {
            required: true,
            validator: (rule, value, callback) =>
              validatePhone(rule, value, callback, self),
            trigger: "blur"
          }
        ],
        type: [
          {
            required: true,
            validator: (rule, value, callback) =>
              validateType(rule, value, callback, self),
            trigger: "blur"
          }
        ],
        date: [
          {
            required: true,
            validator: (rule, value, callback) =>
              validateDate(rule, value, callback, self),
            trigger: "blur"
          }
        ]
      },
      ...
      _.forEach(result.data, item => {
        msg.push(
          h(
            "div",
            null,
            `类型: ${item.type} - 价格: ${item.price} - 数量: ${
              item.count
            } - 总价: ${item.total}元 - 缴费时间: ${
              item.pay_date ? item.pay_date : "未缴费"
      ...
      this.$msgbox({
        ...
        .then(action => {
          this.$message({
            ...
            message:
              this.selectValue == "1"
                ? this.$t("manage.showMessage.operateSuccess")
                : this.$t("manage.showMessage.successSend")
          });
        })
        .catch(() => {});
    },
    ...
    deleteBill(row) {
      ...
      self
        ...
        .then(async () => {
          ...
          if (_.isEqual(response.data, "success to delete")) {
            self.getAllBill();
            self.showSuccessMessageBox();
          } else {
            self.showErrorMessageBox();
